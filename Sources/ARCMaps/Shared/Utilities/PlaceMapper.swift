...
/// ```swift
/// // 1. Search for places
/// let results = try await viewModel.searchPlaces(query: PlaceSearchQuery(name: "coffee"))
///
...
public enum PlaceMapper {
    /// Converts a search result to a map place.
    ///
    ...
    ///
    /// - Parameter result: The search result to convert.
    /// - Returns: A `MapPlace` ready to be displayed on the map.
    public static func toMapPlace(_ result: PlaceSearchResult) -> MapPlace {
