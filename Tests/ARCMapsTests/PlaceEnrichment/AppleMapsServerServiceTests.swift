...
@Suite(.serialized) struct AppleMapsServerServiceTests {
    ...
    @Test("Search places returns mapped results") func searchPlacesReturnsMappedResults() async throws {
        // Given
        let place1 = AppleMapsServerPlace(placeId: "server_place_1",
        ...
        let place2 = AppleMapsServerPlace(placeId: "server_place_2",
                                          name: "El Café",
        ...
        let response = AppleMapsServerSearchResponse(results: [AppleMapsServerSearchResult(place: place1),
    ...
    func searchPlacesFiltersInvalidResults() async throws {
        ...
        let invalidPlace = AppleMapsServerPlace(placeId: nil,
        ...
        let validPlace = AppleMapsServerPlace(placeId: "valid_place",
                                              name: "Valid Place",
                                              formattedAddressLines: ["Address"],
                                              coordinate: AppleMapsServerCoordinate(latitude: 40.0, longitude: -3.0),
                                              pointOfInterestCategory: "restaurant")
        let response = AppleMapsServerSearchResponse(results: [AppleMapsServerSearchResult(place: invalidPlace),
    ...
    @Test("Search places caches results after fetch") func searchPlacesCachesResultsAfterFetch() async throws {
        // Given
        let place = AppleMapsServerPlace(placeId: "cache_test",
                                         name: "Cache Test Place",
                                         formattedAddressLines: ["Address"],
                                         coordinate: AppleMapsServerCoordinate(latitude: 40.0, longitude: -3.0),
                                         pointOfInterestCategory: "restaurant")
        let response = AppleMapsServerSearchResponse(results: [AppleMapsServerSearchResult(place: place)])
        await mockNetworkClient.setMockResponse(response)
